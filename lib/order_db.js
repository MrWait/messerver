...
order_db.savemeterial_online = function(order){
  var defer = Q.defer();
  console.log(order.mid);
  console.log(order.count);
  client.hset('factory_online', order.mid, Number(order.count), function(err, data){
    defer.resolve(data);
  });
  return defer.promise;
};

order_db.getmeterial_online = function(orderid){
  var defer = Q.defer();
  client.hget('factory_online', orderid, function(err, data){
    defer.resolve(data);
  });
  return defer.promise;
};

order_db.getallmeterial_online = function(){
  var defer = Q.defer();
  client.hgetall('factory_online', function(err, data){
    if(err){
      defer.reject(err);
    }else{
      defer.resolve(data);
